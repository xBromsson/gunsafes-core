<?php
/**
 * Admin UI for manually ordering direct child categories on parent categories.
 *
...
if ( ! class_exists( 'GScore_Category_Child_Order_Admin' ) ) {
	class GScore_Category_Child_Order_Admin {
		private const TAXONOMY = 'product_cat';
		private const META_KEY = 'gscore_child_category_order';
		private const NONCE_ACTION = 'gscore_save_category_child_order';
		private const NONCE_NAME = 'gscore_category_child_order_nonce';
		...
		public function render_edit_field( $term, $taxonomy ): void {
			...
			<tr class="form-field gscore-term-order-field">
				<th scope="row">
					<label for="gscore-category-child-order"><?php esc_html_e( 'Direct Child Category Order', 'gunsafes-core' ); ?></label>
				</th>
				<td>
					...
					<?php if ( empty( $children ) ) : ?>
						<p class="description">
							<?php esc_html_e( 'This category does not currently have any direct child categories to order.', 'gunsafes-core' ); ?>
						</p>
					<?php else : ?>
						<p class="description">
							<?php esc_html_e( 'Drag and drop the direct child categories into the order you want the child category grid to use on this parent category archive.', 'gunsafes-core' ); ?>
						</p>
						...
						<div class="gscore-term-sorter" data-default-order="<?php echo esc_attr( $this->get_alphabetical_order_csv( $children ) ); ?>">
							<div class="gscore-term-sorter__toolbar">
								<input
									...
									placeholder="<?php esc_attr_e( 'Filter child categories…', 'gunsafes-core' ); ?>"
								/>
							...
							<input
								type="hidden"
								id="gscore-category-child-order"
								name="gscore_category_child_order"
		...
		public function save_term( $term_id ): void {
			...
			$raw_order = isset( $_POST['gscore_category_child_order'] ) ? wp_unslash( $_POST['gscore_category_child_order'] ) : '';
			$ordered   = array();
